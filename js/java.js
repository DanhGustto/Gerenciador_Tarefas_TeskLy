...
function adicionarTarefaAoContainer(tarefa) {
    ...
    if (container) { // VERIFICAÇÃO DE EXISTÊNCIA - Confirma que container foi encontrado
        ...
        // DEFINIÇÃO DA CLASSE CSS BASEADA NA CATEGORIA - Aplica estilo específico para cada seção
        let classeCSS = 'tarefa-item'; // CLASSE PADRÃO - Estilo base para todas as tarefas
        switch(tarefa.categoria) {
            case 'hoje':
                classeCSS = 'tarefa-item-hoje';
                break;
            case 'prox':
                classeCSS = 'tarefa-item-prox';
                break;
            case 'projetos':
                classeCSS = 'tarefa-item-projetos';
                break;
            default:
                classeCSS = 'tarefa-item';
        }
        
        tarefaElement.className = classeCSS; // APLICAÇÃO DE ESTILO - Define classe CSS para formatação visual
        tarefaElement.innerHTML = ` 
            <h4>${tarefa.titulo}</h4> 
            <p>${tarefa.descricao}</p> 
            <small>Prazo: ${tarefa.prazo} | Prioridade: ${tarefa.prioridade}</small> 
        `;
